...
Further, the \pkg{survreg} function generally gives $\log \sigma$, rather than $\sigma$ as output. The function \pkg{WeibullReg} (along with \pkg{ConvertWeibull}) solves this problem. Hazard ratios ($\exp{(\boldsymbol{\beta}_i)}$) are additionally produced.
...
Most of the work of the function is actually performed by \pkg{ConvertWeibull}. These functions require the \pkg{survival} package in \prog{R}. Formulas for the variance estimates come from \cite{klein_03} (Equations 12.2.13-18, with some modifications since \prog{R} gives $\log \sigma$).


\section{\pkg{WeibullDiag}}
The \pkg{WeibullDiag} function produces a diagnostic plot for Weibull AFT regression, similar to what is present 
